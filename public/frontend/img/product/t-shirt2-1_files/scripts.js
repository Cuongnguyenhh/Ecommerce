...
(function() {
  if (!__sections__["hashtag-feed"]) return;
  try {
    
const feed = document.querySelector('.hashtag-feed')
fetch('https://feeds.behold.so/kg2r1941nzmfI0ljI61N')
	.then(data => data.json())
	.then(response => {
	  response.media.forEach(({
		id,mediaUrl,permalink,caption,mediaType,timestamp,children,commentsCount,likeCount,hashtag,
	  }) => {
		const newItem = document.createElement('a')
		newItem.classList.add('feed-item')
		newItem.href = permalink
